...
export function FabricCareCard() {
  ...
  return (
    <Card size="small">
      ...
      <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 5, fontSize: 12 }} id="care-label">
        Уход за материалом
      </Typography.Text>
      <Radio.Group
        value={care}
        onChange={(e) => dispatch(setCare(e.target.value as CareType))}
        style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5, width: '100%' }}
      ...
      >
        {CARE_OPTIONS.map(([value, { title, subtitle }]) => (
          <Radio.Button
            key={value}
            value={value}
            style={{ textAlign: 'center', height: 'auto', padding: '5px 4px', lineHeight: 1.15 }}
          >
