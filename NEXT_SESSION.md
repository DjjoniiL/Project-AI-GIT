...
- ❌ **Этап 2 (Frontend)** — следующий шаг проекта. `apps/frontend/src/App.tsx` — пока плейсхолдер. **Важно**: вкладка `CRM_DEAL_DETAIL_TAB` уже привязана и технически откроется в карточке сделки на портале, но задеплоенный бэкенд пока отдаёт только API (`/api/health`, `/api/order`) — реального конструктора на экране пользователь пока не увидит, это ожидаемо до реализации Этапа 2.

## Дальше — Этап 2 (Frontend/SVG-конструктор, specification.md раздел 8)

Этап 1 полностью закрыт (см. выше) — весь бэкенд живёт на реальной инфраструктуре VibeCode и готов принимать запросы. Следующий шаг — перенести UI из `design/maket-6.html` в реальные React-компоненты `apps/frontend` (Ant Design, Redux Toolkit), см. specification.md раздел 8, Этап 2. После готовности фронтенда — собрать и задеплоить его тоже (либо статикой на отдельном `static`-рантайме VibeCode, либо отдать через тот же Express-бэкенд).
