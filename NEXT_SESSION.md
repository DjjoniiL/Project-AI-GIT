...
- 🔶 **Этап 1 (Backend/BFF) — почти завершён.** Всё выше плюс код: `apps/backend/src/{vibecode/client.ts, middleware/identity.ts, services/deals.ts, services/disk.ts, routes/order.ts}` — реализованы, протестированы (17/17 зелёных на моках `fetch`), собираются и работают на реальном Node 20. **Единственное, что осталось — `POST /v1/placements/bind`** (см. ниже, единственный шаг, которому нужно участие пользователя).
- ❌ **Этап 2 (Frontend)** — не начат (`apps/frontend/src/App.tsx` — плейсхолдер с Ant Design/Redux Toolkit).

## Дальше — последний шаг Этапа 1: `placements/bind`

Всё остальное из плана Этапа 1 (specification.md, раздел 8) сделано — см. выше. Осталось только встраивание в портал:

- `POST /v1/placements/bind` ключом `vibe_app_` требует **пользовательскую сессию** (`Authorization: Bearer <session>`), а не просто ключ — подтверждено живым `GET /v1/me` с `vibe_app_` ключом: `servers.create` тоже требовал сессию, но там был обходной путь через личный `vibe_api_` ключ (сработало); для **bind именно OAuth-приложения** такого обхода нет — нужен реальный пользователь портала, прошедший OAuth (`POST /v1/oauth/authorize` → `/v1/oauth/token`), либо открывший приложение через портал `vibecode02.bitrix24.ru` и авторизовавший его.
- **Это единственный шаг, который агент не может сделать сам** — нужно от пользователя: либо открыть портал `vibecode02.bitrix24.ru`, поставить приложение "Конструктор заказов" (app id `e7709e0a-6788-471e-b733-37ff9526d885`) через раздел разработки приложений, авторизовать его от своего имени, либо явно попросить агента провести OAuth flow пошагово с интерактивным участием пользователя (переход по ссылке авторизации в браузере).
- После получения сессии — `POST /v1/placements/bind {"placement":"CRM_DEAL_DETAIL_TAB","handler":"https://app-2a2f90ace69d.vibecode.bitrix24.tech","title":"Конструктор заказов"}` (handler — уже задеплоенный appUrl).
...
- PDF-экспорт бланка заказа (кнопка «Скачать файл заказа в PDF») в макете — client-side демо на jsPDF, ВРЕМЕННО. Реальный экспорт — бэкенд + Puppeteer/HTML-шаблон (**подтверждено рабочим, см. выше — fallback на `pdf-lib` больше не нужен**, но не забыть добавить apt-пакеты для Chromium в install-команду боевого деплоя). AVIF→PDF пайплайн сжатия макетов (specification.md, раздел 4.1) — тоже пока клиентская имитация, реальное AVIF-кодирование (`@jsquash/avif` — кандидат) — Этап 2.
