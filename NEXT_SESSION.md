...
- 🔶 **Этап 1 (Backend/BFF) — в работе.** Реализовано и покрыто тестами (`npm test --workspace apps/backend` — зелёные, `npm run build`/`npm run lint` чисто):
  ...
  Все тесты — на моках `fetch`, живых вызовов к VibeCode API ещё не было: в одной из сессий `vibecode.bitrix24.tech` был недоступен из окружения агента (DNS резолвился, TCP на 443 таймаутил; у пользователя сайт открывался нормально — проблема сетевого пути именно из среды агента). **Первым делом в новой сессии — проверить, восстановилась ли сеть** (см. шаг 1 ниже).
- ❌ **Этап 2 (Frontend)** — не начат (`apps/frontend/src/App.tsx` — плейсхолдер с Ant Design/Redux Toolkit).

## Дальше — 8 шагов Этапа 1 (specification.md, раздел 8)

Шаги 4–6 сделаны (см. выше). Остались:

1. **Проверить сеть**: `curl -s -o /dev/null -w "%{http_code}" https://vibecode.bitrix24.tech/v1/me`. Если не `000` — выполнять живыми запросами шаги 2–3 ниже.
...
4. Собрать `POST /api/order` — валидация входа + multipart-приём файла (понадобится `multer`, ещё не добавлен как зависимость) + вызов `createOrUpdateDeal`/`uploadFileToDisk`.
...
apps/backend/src/
  vibecode/client.ts, middleware/identity.ts, services/deals.ts, services/disk.ts — реализованы и протестированы
